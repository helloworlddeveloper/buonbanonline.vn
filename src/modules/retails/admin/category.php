...
if ( $nv_Request->isset_request( 'ajax_action', 'post' ) ) {
        ...
        while ( $row = $result->fetch() )
 {
            ...
            $sql = 'UPDATE ' . TABLE . '_categories SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query( $sql );
        }
...
if ( $nv_Request->isset_request( 'delete_id', 'get' ) and $nv_Request->isset_request( 'delete_checkss', 'get' ) ) {
    ...
    if ( $id > 0 and $delete_checkss == md5( $id . NV_CACHE_PREFIX . $client_info['session_id'] ) ) {
		...
		// kiểm tra categories có chuyên mục con thì không được phép xóa chuyên mục cha. vì mất kết nối.
		$count_subcatid = $db->query('SELECT count(*) as count FROM '. TABLE .'_categories WHERE parrent_id ='. $id)->fetchColumn();
...
if ( !$nv_Request->isset_request( 'id', 'post,get' ) ) {
    ...
    ->where( 'idsite=0 and parrent_id='.$parrent_id.$where );
...
$xtpl->assign( 'ROW', $row );
$xtpl->assign( 'Q', $q );
$xtpl->assign( 'category_add', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=category_add&parrent_id='.$parrent_id );
if ( $ngay_tu > 0 )
    ...
    while ( $view = $sth->fetch() ) {
            ...
            foreach ( $array_inhome as $key => $val ) {
                $xtpl->assign( 'INHOME', array(
                    'key' => $key,
                    'title' => $val,
                    'selected' => $key == $view['inhome'] ? ' selected=\'selected\'' : ''
                ) );
            ...
            foreach ( $array_viewcat as $key => $val ) {
                $xtpl->assign( 'VIEWCAT', array(
                    'key' => $key,
                    'title' => $val,
                    'selected' => $key == $view['viewcat'] ? ' selected=\'selected\'' : ''
                ) );
                $xtpl->parse( 'main.view.loop.viewcat' );
            }
            for ( $i = 0; $i <= 10; $i++ ) {
			...
			$view['count']=count_category($view['id']);
            $xtpl->assign( 'CHECK', $view['status'] == 1 ? 'checked' : '' );
			$view['link_view']=NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=category&amp;parrent_id=' . $view['id'];
            $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=category_add&amp;id=' . $view['id'];
